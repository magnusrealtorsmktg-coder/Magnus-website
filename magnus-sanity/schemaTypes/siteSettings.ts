import {defineField, defineType} from 'sanity'

/** Single settings document: contact details, social links, section intros. */
export const siteSettings = defineType({
  name: 'siteSettings',
  title: 'Site Settings',
  type: 'document',
  fields: [
    defineField({
      name: 'heroImage',
      title: 'Hero image (main)',
      type: 'image',
      description: 'The large landscape photo on the right of the top banner. Drag the focus point so the key part stays visible when cropped.',
      options: {hotspot: true},
    }),
    defineField({name: 'heroSub', title: 'Hero — description paragraph', type: 'text', rows: 3,
      description: 'The intro paragraph under the big hero headline (e.g. "Helping families, investors, and businesses discover the right opportunities…"). Leave empty to keep the built-in text.'}),
    defineField({name: 'heroCard1Label', title: 'Hero floating card 1 — label', type: 'string',
      description: 'The small caption on the first floating card over the hero photo, e.g. "Capital Growth".'}),
    defineField({name: 'heroCard1Value', title: 'Hero floating card 1 — value', type: 'string',
      description: 'The big number/text on the first floating card, e.g. "18% · 3 yr".'}),
    defineField({name: 'heroCard2Label', title: 'Hero floating card 2 — label', type: 'string',
      description: 'The small caption on the second floating card over the hero photo, e.g. "Active Listings".'}),
    defineField({name: 'heroCard2Value', title: 'Hero floating card 2 — value', type: 'string',
      description: 'The big number/text on the second floating card, e.g. "120+".'}),
    defineField({
      name: 'aboutImageBig',
      title: 'About section — main image',
      type: 'image',
      description: 'The large photo in the "About Magnus" section. Optional.',
      options: {hotspot: true},
    }),
    defineField({
      name: 'aboutImageSmall',
      title: 'About section — small inset image',
      type: 'image',
      description: 'The smaller photo that overlaps the main About image. Optional.',
      options: {hotspot: true},
    }),
    defineField({
      name: 'whyImage',
      title: 'Why Magnus — image',
      type: 'image',
      description: 'The photo shown beside the "Why Magnus" list. Optional.',
      options: {hotspot: true},
    }),
    defineField({name: 'phone', title: 'Phone number', type: 'string',
      description: 'Full international format, e.g. "+919820420547". Used for tap-to-call AND WhatsApp.'}),
    defineField({name: 'email', title: 'Email', type: 'string'}),
    defineField({name: 'instagram', title: 'Instagram URL', type: 'url',
      description: 'Your Instagram profile link. Powers the Instagram icon in the footer.'}),
    defineField({name: 'facebook', title: 'Facebook URL', type: 'url',
      description: 'Your Facebook page link. Powers the Facebook icon in the footer.'}),
    defineField({name: 'linkedin', title: 'LinkedIn URL', type: 'url'}),
    defineField({name: 'youtube', title: 'YouTube URL', type: 'url',
      description: 'Your YouTube channel or video link. Powers the YouTube icon in the footer.'}),
    defineField({
      name: 'stats',
      title: 'Key numbers (the 3 stats under the hero)',
      type: 'array',
      description: 'The three highlighted numbers under the hero (e.g. Happy Clients, Properties Assisted, Years of Experience). Edit, reorder, add or remove. Leave empty to keep the built-in values.',
      of: [{
        type: 'object',
        fields: [
          {name: 'value', title: 'Number', type: 'number', description: 'Just the digits, e.g. 1500 (it animates and gets commas automatically).'},
          {name: 'suffix', title: 'Suffix', type: 'string', description: 'Shown right after the number, e.g. "+". Optional.'},
          {name: 'label', title: 'Label', type: 'string', description: 'The caption under the number, e.g. "Happy Clients".'},
        ],
        preview: {
          select: {value: 'value', suffix: 'suffix', label: 'label'},
          prepare({value, suffix, label}) {
            // title must be a string — `value` is a number, which would crash the list preview
            const num = value == null ? '' : String(value) + (suffix || '')
            return {title: num || '(no number)', subtitle: label || ''}
          },
        },
      }],
    }),
    defineField({name: 'aboutHeading', title: 'About section — heading', type: 'string',
      description: 'The bold headline in the "About Magnus" section. Wrap any words in *asterisks* to show them in gold italic, e.g. "Local expertise. *Professional advice.* Lasting relationships."'}),
    defineField({name: 'aboutText', title: 'About section — paragraph', type: 'text', rows: 5,
      description: 'The descriptive paragraph shown under the About heading.'}),
    defineField({name: 'aboutPoints', title: 'About section — checklist points', type: 'array', of: [{type: 'string'}],
      description: 'The ticked points (e.g. "Deep local market understanding"). Add, remove or reorder freely. Leave empty to keep the built-in list.'}),
    defineField({name: 'teamMagnusHeading', title: 'Team Magnus — heading', type: 'string',
      description: 'The headline of the "Team Magnus" section. Wrap any words in *asterisks* to show them in gold italic, e.g. "A team that puts *your interests first*".'}),
    defineField({name: 'teamMagnusText', title: 'Team Magnus — paragraph', type: 'text', rows: 5,
      description: 'The descriptive paragraph shown in the "Team Magnus" section.'}),
    defineField({name: 'ownerIntro', title: 'Owner section intro', type: 'text', rows: 3,
      description: 'The paragraph under "Meet the Owners".'}),
  ],
  preview: {prepare: () => ({title: 'Site Settings'})},
})
